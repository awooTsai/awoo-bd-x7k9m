...
使用 Google Drive API（透過環境變數 GOOGLE_APPLICATION_CREDENTIALS 中的 Service Account 認證），讀取 Google Drive 資料夾 ID: `1Zud6lHXA8Iuqpc0LoVH3mOniMWj4fDJa`。

### 重要：完整掃描所有檔案

1. **列出資料夾中所有檔案**（包含子資料夾），不要只看根目錄
2. **也列出子資料夾的內容**，遞迴掃描
3. 列出每個檔案的：名稱、建立日期、修改日期、MIME type
4. **先列出完整清單**，再進行篩選

### 日期篩選邏輯（寬鬆比對）

目標日期由環境傳入。篩選時用以下任一條件命中即可：
- 檔案名稱中包含目標日期（任何格式：2026-03-27、03/27、0327、3/27）
- 檔案的 **createdTime** 或 **modifiedTime** 落在目標日期當天（台灣時區 UTC+8）
- 檔案內容中提到目標日期

### 讀取檔案內容

對於命中的每個檔案（Google Doc / PDF / 文字檔）：
- 使用 Google Drive API 的 `export` 功能將 Google Doc 匯出為純文字
- 讀取完整內容，不要只看標題

### 結束條件
- 只有在**完全確認沒有任何目標日期的逐字稿**後，才結束任務不做 commit
- 如果有任何不確定，寧可讀取檔案內容來確認

## 第三步：解析四位 AE 的會議內容

只處理有以下**四位 AE 發言**的會議：**Leon（呂紹平）、Aiden（蔡毓倫）、Bryan（周至剛）、Rebecca（陳玟君）**

注意：逐字稿中可能用英文名或中文名，兩者都要辨識。

從每份逐字稿提取以下資訊：

| 欄位 | 說明 |
|------|------|
| AE 歸屬 | **關鍵規則：統計每位 AE 在該會議中的發言次數和字數，發言最多的 AE 就是該客戶的負責人。** 不要只看會議標題或參與者列表，必須分析實際對話內容來判斷。如果一場會議有多位 AE 參與，以發言量最大的那位為主要負責人。 |
| 客戶名稱 | 公司/品牌名 + 簡短描述 |
| 會議日期 | MM/DD 格式 |
| 會議時長 | N min 格式 |
| 參與者 | 所有與會者姓名 |
